...
```bash
./Scripts/package.sh 0.0.3
```

This creates `dist/PasteX-v0.0.3-macos-arm64.zip`, containing an ad-hoc signed `PasteX.app` for Apple Silicon Macs. To sign with a Developer ID for external distribution, provide your identity:

```bash
SIGNING_IDENTITY="Developer ID Application: Your Name (TEAMID)" ./Scripts/package.sh 0.0.3
```

The GitHub `v0.0.3` release includes the generated ZIP and its SHA-256 checksum.
